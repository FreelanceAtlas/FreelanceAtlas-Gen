...
// post-generation fact-check (factCheckArticle, run fresh in route.ts after generateArticle
// returns), not just the internal self-correction loop's own bestCheck. Live retesting found
// a real gap here: bestCheck is one specific fact-check LLM call made *inside* generateArticle;
// route.ts's factCheck is a separate, later LLM call against the same final text. The two don't
// always agree — a Trello "10 collaborators" / "10 boards" / "250 workspace command runs per
// month" fabrication that bestCheck didn't flag as HIGH (or flagged too late, after the self-
// correction budget/attempt cap was hit) sailed through this function entirely, because it only
// ever saw bestCheck.issues, never the authoritative issues that actually got persisted and
// shown to the editor as needs_review. Since this function is a pure, idempotent text transform
// keyed off whatever issues list it's given, it's safe to call again downstream with a different,
// more authoritative issues list — re-redacting figures already caught the first time is a no-op
// (toRedact just won't find that literal substring anymore), and it additionally catches whatever
// the first pass missed.
// Round 11: a live retest on the Asana/Trello/ClickUp "solo freelance consultants" topic found
// a real false-negative this protectedKeys mechanism let through unredacted: a HIGH severity
// issue flagged "$10 per month on a monthly basis" as an unconfirmed ClickUp Unlimited monthly
// price (no fetched source confirms it), but the article still published "$10 per month"
// verbatim because a separate LOW severity issue elsewhere in the same fact-check result
// confirmed "Trello Premium is $10 per user per month billed annually" as accurate. Both
// figures normalize to the exact same value ("$10") via DEFAULT_FIGURE_NORMALIZER, which has
// no concept of which company a figure belongs to — so the genuinely-verified Trello $10
// incorrectly protected the genuinely-fabricated ClickUp $10 from redaction, purely because
// they happened to be the same number for two different products.
//
// This mirrors a problem Round 6 already solved once, for a different part of this file
// (buildVerifiedFigureSetByCompany / findCompanyInContext, used by
// redactUnverifiedFiguresFromArticle above): a verified figure should only protect an
// unverified one when they're actually about the same company, not just the same number. The
// fix here reuses that same findCompanyInContext lookup, applied to the issue's own claim/
// concern text (which almost always names the product the figure belongs to, e.g. "Trello
// Premium is $10..."), to scope protection by company+value instead of value alone. When a
// company can't be identified in either an issue's text, this falls back to the old
// value-only behavior (ambiguous cases stay exactly as conservative as before — see the Round
// 8 comment below for why over-redacting is the other failure mode this guards against).
function buildCompanyKeys(fetchedSources: Record<string, string>): string[] {
  const keys = new Set<string>();
  for (const url of Object.keys(fetchedSources)) {
    const key = deriveCompanyKey(url);
    if (key) keys.add(key);
  }
...
export function redactFiguresFlaggedByFactCheck(
  ...
  // Augment hostname-derived company keys with tokens from the topic hint (Round 12).
  // Needed when the AI fetched third-party aggregator pages instead of a vendor's own
  // pricing page — in that case no source URL hostname contains the vendor name.
  const companyKeys = [
    ...buildCompanyKeys(fetchedSources),
    ...topicHint.toLowerCase().split(/\s+/).filter((t) => t.length >= 3),
  ];
  const protectedKeys = new Set<string>();
  const protectedByCompany = new Set<string>();
  for (const issue of issues) {
    if (issue.severity === "high") continue;
    for (const text of [issue.claim, issue.concern]) {
      const company = findCompanyInContext(text, companyKeys);
      for (const { pattern, normalize, extractLiteral } of FLAGGED_FIGURE_PATTERNS) {
        for (const rawMatch of text.match(pattern) ?? []) {
          ...
          if (company) protectedByCompany.add(`${company}|${value}`);
        }
      }
    }
  }
    ...
    for (const text of [issue.claim, issue.concern]) {
      ...
      for (const { pattern, placeholder, normalize, extractLiteral } of FLAGGED_FIGURE_PATTERNS) {
        ...
        for (const rawMatch of matches) {
          const literal = extractLiteral ? extractLiteral(rawMatch) : rawMatch;
          const value = normalize(literal);
          // When the HIGH issue's own text names a company, only let a same-company LOW/MEDIUM
          // mention of the identical value protect it — that's the actual Round 11 fix. When no
          // company is identifiable here (ambiguous, e.g. a cross-company Key Takeaways line),
          // fall back to the old value-only check rather than risk under-protecting a genuinely
          // ambiguous case.
          const isProtected = company
            ? protectedByCompany.has(`${company}|${value}`)
...
// ── Redo: targeted fix of a failing draft's flagged issues ───────────────────
// Surgically fixes only the passages/claims the originality + fact-check flagged,
// leaving everything else verbatim (a full rewrite was introducing new issues and
// lowering the score). Crucially, it runs with web_fetch enabled so the model can
// pull the actual source pages to verify/correct specific figures, and it returns
// the fetched page text so the caller can re-score the fact-check against ground
// truth (without it, every specific number gets flagged "unverifiable"). Also
// fixes the FAQs, which the fact-check scores too. Fail-soft: on any failure it
// returns the original content/faqs unchanged with rewritten:false.
